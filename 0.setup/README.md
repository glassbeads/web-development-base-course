...
<details>
    <summary>Почему именно .venv?</summary>

Это распространённое соглашение, используемое большинством разработчиков.

Названия с `.` - практика, начавшаяся в операционных системах семейства Unix. В них, файлы и папки, названия которых начинаются с `.`, скрыты. В такие файлы и папки обычно помещают вещи, которые лучше скрывать от глаз обычных пользователей - это какие-то настройки софта, [кэши](https://en.wikipedia.org/wiki/Cache_(computing)), прочее.

Название директории с `.` в проекте гарантирует, что в дереве проекта эта директория будет в самом верху, и не будет смешиваться с другими папками, в которых будет лежать код.

Другие распространённые примеры таких папок: `.git`, `.idea`, `.vscode`.

Также есть и файлы, начинающиеся с точки, разработчики очень любят такие файлы и плодят их в большом количестве. В них обычно хранится конфигурация. Примеры распространённых файлов: `.gitignore`, `.dockerignore`, `.helmignore`, `.env`, `.npmrc`, `.htaccess`, `.gitlab-ci.yml`, `.behaverc`, тысячи их.
</details>
